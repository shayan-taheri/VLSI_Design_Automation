...
CostType SimpleGR::routeMaze(const IdType netId, bool allowOverflow,
	...
	/* "closetset": This vector has been defined for keeping IDs of the Tiles that has been used for routing of each net.
	...
	if ((xx + tileWidth) <= max(net.one.x, net.two.x) && ((xx + tileWidth) < xTiles)) {
	...
	* But if the overflow is not allowable, then if the edge between two tile has an usage lower than its capacity, consider it. */
	...
	if ((xx - tileWidth) >= min(net.one.x, net.two.x) && ((xx - tileWidth) < xTiles)) {
	...
	* But if the overflow is not allowable, then if the edge between two tile has an usage lower than its capacity, consider it. */
	...
	if ((yy + tileHeight) <= max(net.one.y, net.two.y) && ((yy + tileHeight) < yTiles)) {
	...
	* But if the overflow is not allowable, then if the edge between two tile has an usage lower than its capacity, consider it. */
	...
	if ((yy - tileHeight) >= min(net.one.y, net.two.y) && ((yy - tileHeight) < yTiles)) {
	...
	for (IdType cc = 0; cc <= (neighbors_id.size() - 1); cc++) {
		...
		cont_c = 0;
		for (IdType dd = 0; dd <= (closedset.size() - 1); dd++) {
		...
		tmp_nigh = neighbors_id[cc];
		if (priorityQueue.visited(tmp_nigh) == true) {
		cont_o = 1;}

		if (cont_o == 1) { 
		if (tentative_totalCost >= priorityQueue.getData(tmp_nigh).totalCost) {
			continue;
		}}
	...
	for (IdType ti = 0; ti <= (closedset.size() - 1); ti ++) {
